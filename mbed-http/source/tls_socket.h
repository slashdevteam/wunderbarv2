...
public:
    TLSSocket(NetworkInterface* net_iface, const char* hostname, uint16_t port, const char* ssl_ca_pem) {
        ...
        mbedtls_entropy_add_source(&_entropy,
                               pollEntropyTLS,
                               NULL,
                               128,
                               0); // weak entropy source @TODO: check for strong source?
        _alpn_list[0] = "h2";
        _alpn_list[1] = "http/1.1";
        _alpn_list[2] = nullptr;
    }
    ...
    nsapi_error_t connect() {
        ...
        mbedtls_ssl_conf_alpn_protocols(&_ssl_conf, _alpn_list);
#if DEBUG_LEVEL > 0
